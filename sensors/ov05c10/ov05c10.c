...
int32_t OV05C10_ReadID(OV05C10_Object_t *pObj, uint32_t *Id) {
	...
	if ((pObj == NULL) || (Id == NULL) ||
		(pObj->IO.Init == NULL) || (pObj->IO.ReadReg == NULL)) {
		return OV05C10_ERROR;
	}
	...
	/* Read chip ID directly from page-0 ID registers */
	if (ov05c10_read_reg(&pObj->Ctx, OV05C10_REG_ID_BYTE_3, &tmp[3], 1) != OV05C10_OK) {
		ret = OV05C10_ERROR;
	} else if (ov05c10_read_reg(&pObj->Ctx, OV05C10_REG_ID_BYTE_2, &tmp[2], 1) != OV05C10_OK) {
		ret = OV05C10_ERROR;
	} else if (ov05c10_read_reg(&pObj->Ctx, OV05C10_REG_ID_BYTE_1, &tmp[1], 1) != OV05C10_OK) {
		ret = OV05C10_ERROR;
	} else if (ov05c10_read_reg(&pObj->Ctx, OV05C10_REG_ID_BYTE_0, &tmp[0], 1) != OV05C10_OK) {
		ret = OV05C10_ERROR;
	}

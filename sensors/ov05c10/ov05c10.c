...
static const struct regval ov05c10r1a_ae02_init_2888x1808_2lane_raw10_30fps[] = {
	...
	{0x0103, 0x00},
	{0x0104, 0x06},
	{0x0105, 0x06}, /* VTS, approximately 30.5 fps */
	{0x0106, 0xF4}, /* VTS, approximately 30.5 fps */
	{0x0107, 0x08},
	{0x011B, 0x01},
	{0x0124, 0xFF},
	{0x0142, 0x5D},
	...
	{0x02A3, 0x10},
	{0x02A5, 0x02},
	{0x02A6, 0x0B}, /* hsize[11:8] */
	{0x02A7, 0x48}, /* hsize[7:0] */
	...
	{0x00E7, 0x00},
	{0x00FD, 0x00},
	{0x008E, 0x0B}, /* hsize[11:8] */
	{0x008F, 0x48}, /* hsize[7:0] */
	{0x0093, 0x18},
	{0x0094, 0xFF},
	{0x0095, 0xBD},
	{0x0096, 0x1A},
...
static int32_t OV05C10_GetExposureRange(OV05C10_Object_t *pObj, uint32_t *min_us,
	...
	if (ov05c10_read_reg(&pObj->Ctx, OV05C10_REG_VTS_MSB, vts_raw, 3) != 0) {
		ret = OV05C10_ERROR;
		goto exit_exp_range;
	}
...
int32_t OV05C10_SetExposure(OV05C10_Object_t *pObj, int32_t exposure_us) {
	...
	if (ov05c10_read_reg(&pObj->Ctx, OV05C10_REG_VTS_MSB, vts_raw, 3) != OV05C10_OK) {
		ret = OV05C10_ERROR;
		goto exit_exp;
	}
	hts = SWAP_ENDIAN16(hts);
	...
	if(vts_new != vts) {
		...
		if (ov05c10_write_reg(&pObj->Ctx, OV05C10_REG_VBLANK, (uint8_t*) &vblank, 2) != OV05C10_OK) {
			ret = OV05C10_ERROR;
			goto exit_exp;
		}
		if (ov05c10_write_reg(&pObj->Ctx, OV05C10_REG_VBLANK_TRIGGER, &trigger, 1) != OV05C10_OK) {
			ret = OV05C10_ERROR;
			goto exit_exp;
		}
	}

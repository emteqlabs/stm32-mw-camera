...
#define CMW_PIXEL_FORMAT_YUV420_8                                              \
  DCMIPP_DT_YUV420_8 /*!< Data Type YUV420 8bit  */
#define CMW_PIXEL_FORMAT_YUV420_10                                             \
  DCMIPP_DT_YUV420_10 /*!< Data Type YUV420 10bit  */
#define CMW_PIXEL_FORMAT_YUV422_8                                              \
  DCMIPP_DT_YUV422_8 /*!< Data Type YUV422 8bit  */
#define CMW_PIXEL_FORMAT_YUV422_10                                             \
  DCMIPP_DT_YUV422_10                            /*!< Data Type YUV422 10bit */
 ...
 * The following defines allow the user to chose the exposure mode of the
 * camera. Camera exposure mode has no impact if the camera does not support it.
 */
#define CMW_EXPOSUREMODE_AUTO                                                  \
  0x00U /* Start the camera auto exposure functionnality */
#define CMW_EXPOSUREMODE_AUTOFREEZE                                            \
  0x01U /* Stop the camera auto exposure functionnality and freeze the current \
           value */
#define CMW_EXPOSUREMODE_MANUAL                                                \
  0x02U /* Set the camera in manual exposure (exposure is control by a         \
           software algorithm) */
 ...
 * @param  advanced_config  Optional: Sensor specific configuration; NULL if you
 * want to let CMW configure for you
 * @retval CMW status
 */
int32_t CMW_CAMERA_Init(CMW_CameraInit_t *init_conf,
                        CMW_Advanced_Config_t *advanced_config);
 ...
 */
int32_t
CMW_CAMERA_SetDefaultSensorValues(CMW_Advanced_Config_t *advanced_config);
int32_t CMW_CAMERA_DeInit();
int32_t CMW_CAMERA_Run();
int32_t CMW_CAMERA_SetPipeConfig(uint32_t pipe, CMW_DCMIPP_Conf_t *p_conf,
                                 uint32_t *pitch);
...
int32_t CMW_CAMERA_DoubleBufferStart(uint32_t pipe, uint8_t *pbuff1,
                                     uint8_t *pbuff2, uint32_t Mode);
int32_t CMW_CAMERA_Suspend(uint32_t pipe);
int32_t CMW_CAMERA_Resume(uint32_t pipe);
int32_t
CMW_CAMERA_EnableRestartState(ISP_RestartStateTypeDef *ISP_RestartState);
